...
var AttribPool = require('ep_etherpad-lite/static/js/AttributePool');
...
function loadBroadcast(pad, fireWhenAllScriptsAreLoaded/*, BroadcastSlider*/) {
  ...
  /** 
   * Makes use of the global: clientVars
   */
  var padContents = {
    ...
    applySpliceToDivs: function(start, numRemoved, newLines) {
      ...
      for (var i = 0; i < newDivs.length; i++)
      {
        ...
        {
          $("#timeslider-content").prepend(newDivs[i]);
        }
        startDiv = newDivs[i];
      }
  ...
  function callCatchingErrors(catcher, func) {
    console.log ("callCatchingErrors ()");
  ...
  function wrapRecordingErrors(catcher, func) {
    console.log ("wrapRecordingErrors ()");
  ...
  function loadedNewChangeset(changesetForward, changesetBackward, revision, timeDelta) {
    console.log ("loadedNewChangeset ("+revisionInfo.latest+")");
...
/*
  ...
  function goToRevision(newRevision) {
    console.log ("goToRevision ("+newRevision+")");
  ...
  changesetLoader = {
    ...
    handleSocketResponse: function(message) {
      console.log ("handleSocketResponse ()");
    ...
    handleMessageFromServer: function (obj) {
      console.log("handleMessageFromServer:", arguments);
...
exports.loadBroadcast = loadBroadcast;
